...
import { Handshake, MessageSquare, ArrowRight, FileText } from "lucide-react";
...
interface Deal {
  ...
  seller: { name: string; email: string; avatarUrl: string | null };
...
const STAGE_CONFIG: Record<string, { label: string; color: string; step: number }> = {
  ...
  AGREEMENT: { label: "Agreement Review", color: "bg-brand/10 text-brand", step: 3 },
  ESCROW: { label: "Escrow Funding", color: "bg-info/10 text-info", step: 4 },
  TRANSFER: { label: "Asset Handover", color: "bg-success/10 text-success", step: 5 },
  CLOSED: { label: "Acquisition Closed", color: "bg-success/10 text-success", step: 6 },
...
export function BuyerDealsClient({ deals }: { deals: Deal[] }) {
  if (deals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
        <Handshake className="h-12 w-12 text-muted-foreground/40" />
        <h3 className="font-semibold">No active acquisitions</h3>
        <p className="text-sm text-muted-foreground">Make an offer on a listing to start a transaction deal room.</p>
              ...
              <CardContent className="p-5">
                <div className="flex items-start gap-4">
                  <Avatar className="h-10 w-10 shrink-0">
                    <AvatarImage src={deal.seller.avatarUrl ?? undefined} />
                    <AvatarFallback className="text-xs font-bold fmi-gradient text-white">
                      {deal.seller.name.slice(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">Seller: {deal.seller.name}</p>
